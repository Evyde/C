...
void sendemail(char *email,char *body,char *from,char *passwd,char *IP);
int open_socket(struct sockaddr *addr);

int server(char *IP,char *from,char *passwd,char *to,char *title,char *body1)
{
    char *email = to;
    char body[1000];
	sprintf(body,"From: \"RaspBerryPi\"<%s>\r\nTo: \"%s\"<%s>\r\nSubject: %s\r\n\r\n%s",from,to,to,title,body1);
    sendemail(email, body,from,passwd,IP);
    return 0;
}
...
void sendemail(char *email, char *body,char *from,char *passwd,char *IP)
{
    ...
    their_addr.sin_addr.s_addr = inet_addr(IP);//smtp 服务器
